...
export async function POST(req: NextRequest) {
  try {
    ...
    const [inserted] = await sql`
      INSERT INTO goal_responses (goal_id, subcategory_id, response_type, date, session_notes)
      VALUES (${goal_id}, ${subcategory_id ?? null}, ${response_type}, ${date}, ${session_notes ?? null})
      RETURNING id
    `;
    const [row] = await sql`
